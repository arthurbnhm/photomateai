...
function InlineLoader({ className, text }: { className?: string, text: string }) {
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -10 }}
      className={cn(
        "rounded-lg p-4 flex items-center gap-3 bg-blue-50/50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800/50",
        className
      )}
    >
      ...
      <p className="text-blue-800 dark:text-blue-200">{text}</p>
    </motion.div>
  )
} 